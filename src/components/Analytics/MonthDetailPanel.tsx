            ...
            {isLoading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4">
                        <div className="text-center p-3 bg-surface-hover/30 rounded-xl">
                            ...
                            <p className="text-sm font-bold font-mono text-red-400 mt-1">
                                {formatCOPWithSymbol(totalExpenses)}
                            </p>
                        </div>
                        <div className="text-center p-3 bg-surface-hover/30 rounded-xl">
                            <p className="text-[11px] text-muted-foreground">Income</p>
                            <p className="text-sm font-bold font-mono text-green-400 mt-1">
                                {formatCOPWithSymbol(totalIncome)}
                            </p>
                        </div>
                        <div className="text-center p-3 bg-surface-hover/30 rounded-xl">
                            ...
                            <p className={`text-sm font-bold font-mono mt-1 ${netFlow >= 0 ? "text-green-400" : "text-red-400"}`}>
                                {netFlow >= 0 ? "+ " : "- "}{formatCOPWithSymbol(Math.abs(netFlow))}
                            </p>
                        </div>
                    </div>
                    ...
                    {topTransactions.length > 0 && (
                        <div>
                            <p className="text-xs font-semibold text-foreground mb-2">Top Transactions</p>
                            <div className="space-y-0">
                                {topTransactions.map((txn) => (
                                    ...
                                    >
                                        ...
                                        <span className={`text-xs font-mono font-medium ${txn.type === "income" ? "text-green-400" : "text-foreground"}`}>
                            ...
                            <div className="text-center mt-3">
                                ...
                                >
                                    View all {monthName} transactions →
                                </Link>
                            </div>
                        </div>
                    )}
