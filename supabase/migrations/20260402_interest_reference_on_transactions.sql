-- Phase 4: Interest Reference Point Updates
-- After any balance change on an account with interest_rate set,
-- update interest_reference_balance and interest_reference_date
-- so the yield projection restarts from the new balance.

-- Helper: updates interest reference fields if account has interest_rate
CREATE OR REPLACE FUNCTION _update_interest_reference(
  p_account_id uuid,
  p_user_id uuid
) RETURNS void AS $$
BEGIN
  UPDATE accounts
  SET interest_reference_balance = balance,
      interest_reference_date = now()
...
-- Redefine add_transaction_with_splits (adds interest ref update)
...
BEGIN
  ...
  IF p_account_id IS NOT NULL THEN
    ...
    -- Update interest reference after balance change
...
-- Redefine delete_transaction_with_balance (adds interest ref update)
...
BEGIN
  ...
  IF v_account_id IS NOT NULL THEN
    PERFORM _update_interest_reference(v_account_id, p_user_id);
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

-- Redefine update_transaction_with_splits (adds interest ref update)
...
-- Initialize reference fields for accounts that already have interest_rate set
UPDATE accounts
SET interest_reference_balance = balance,
    interest_reference_date = now()
WHERE interest_rate IS NOT NULL
  AND interest_reference_balance IS NULL;
