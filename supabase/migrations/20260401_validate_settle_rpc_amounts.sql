-- Harden settle RPCs: validate positive amounts and verify splits belong
-- to the target account in settle_debts.
...
BEGIN
  ...
  IF p_received_amount <= 0 THEN
    RAISE EXCEPTION 'Invalid received amount: must be greater than zero';
  END IF;
