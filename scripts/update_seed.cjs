...
async function updateSeed() {
  ...
  const { error: delError } = await supabase.from("account_types").delete().neq("name", "INVALID_NAME_TO_MATCH_ALL"); // Delete all
  ...
  const { error: insError } = await supabase.from("account_types").insert([
